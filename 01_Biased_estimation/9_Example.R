...
library(multiColl)
library(glmnet)
library(ElemStatLearn)  # install from .tar.gz
...
for (i in 1:num_iter) {
  ...
  # Ridge
  ridge_model <- cv.glmnet(X_train, y_train, alpha = 0, grouped = FALSE, standardize = FALSE)
  ...
  # Elastic Net
  elastic_model <- cv.glmnet(X_train, y_train, alpha = 0.5, grouped = FALSE, standardize = FALSE)
